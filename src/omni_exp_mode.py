...
_DEMO_MIDDLE = (
    ...
    "- Phrase as non-GUI research (curl/fetch/CLI). Core writes ONE fancy HTML page "
    "with colorful inline SVG + exactly 4 topic sections; facts cited through Aug 2026 "
    "(no TTS / no auto-play deck).\n"
    "- Speak a short summary when TASK_RESULT arrives.\n"
    "\n"
    "TOOLS:\n"
    "- work: demo HTML page (4 topics, SVG, citations). Same wait/result rules.\n"
    "\n"
)

_WORK_TOOL_DESC_DEMO = (
    "Delegate to core: non-GUI research (facts through Aug 2026 with sources), then ONE "
    "stunning local HTML page with colorful inline SVG and exactly 4 topic sections "
    "(no auto-explain / TTS / slide deck). Open that file. Speak a short summary when ready."
)

DEMO_TASK_SYSTEM = (
    "===SUTANDO SYSTEM INSTRUCTIONS===\n"
    "DEMO MODE — one stunning scrollable HTML page (no auto-play deck).\n"
    ...
    "  * Hero: title + one-line thesis + a prominent colorful inline SVG illustration "
    "(diagram / metaphor / process — not a plain icon).\n"
    "  * Exactly FOUR topic sections (h2), each with a short Chinese title (≤16字), "
    "3–5 bullets, and a small related inline SVG accent or mini-diagram.\n"
    "  * Pick the four most useful angles for the asked product/topic "
    "(e.g. 品牌背景 / 产品特点 / 市场对比 / 使用建议 — adapt to the ask).\n"
    "  * Footer: Sources — every major claim must map to a real URL.\n"
    "- ACCURACY / EVIDENCE (required):\n"
    "  * Prefer primary sources (official docs, papers, company blogs, news with date).\n"
    "  * Content must reflect the latest reliable information available through "
    "August 2026; state the as-of month when dates matter (e.g. 「截至 2026-08」).\n"
    "  * Do NOT invent products, metrics, dates, or quotes. If unsure, say so briefly.\n"
    "  * Each section: ≥1 concrete evidence item — working https URL and/or short "
    "quoted snippet (with attribution + link). Prefer 2–4 distinct sources page-wide.\n"
    "  * Links must be real and clickable; quote text in 「」 or \"\" with source name.\n"
    "- LANGUAGE: user-facing copy in Simplified Chinese; proper nouns may stay original.\n"
    "- VISUAL (required — make it fancy / stunning):\n"
    "  * Rich color palette (gradients, accent hues) with strong hierarchy; generous "
    "padding; polished typography (distinctive web fonts via CDN ok).\n"
    "  * Colorful inline SVG required (hero + per-section accents): shapes, flow "
    "arrows, simple charts, or metaphor art — self-contained in the HTML file "
    "(no external image downloads).\n"
    "  * Subtle CSS motion ok (fade/slide on load); avoid clutter and emoji chrome.\n"
    "  * Still readable on phone/laptop; contrast must stay high.\n"

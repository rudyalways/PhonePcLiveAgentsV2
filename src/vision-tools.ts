...
const SCREEN_CAPTURE_PORT = Number(process.env.SCREEN_CAPTURE_PORT ?? '7900');
...
const screenSource: VisionSource = {
	...
	async capture() {
		...
		const res = await fetch(`http://localhost:${SCREEN_CAPTURE_PORT}/capture?format=jpeg&silent=true`, _capTok ? { headers: { 'X-Sutando-Capture-Token': _capTok } } : {});

...
VOICE_AGENT_DEFAULT_PORT = 9900


def resolve_voice_agent_port(workspace_dir: Optional[Path] = None) -> int:
    """Return the port the running voice-agent actually bound.

    voice-agent.ts writes state/voice-agent.json at listen with its real PORT,
    so a non-default PORT is reported correctly instead of probing a hardcoded
    default — the same "running process is the authority" principle
    scripts/sutando-config.sh already applies to voice_ws. Liveness is validated
    by the recorded pid, NOT a time window: the file is written once at startup
    (not refreshed like the heartbeat), so a window would wrongly expire a
    long-running agent. Absent / dead-pid / unreadable -> default port.
    """
    workspace = Path(workspace_dir or WORKSPACE_DIR)
    try:
        with open(workspace / "state" / "voice-agent.json") as f:
            rec = json.load(f)
    except Exception:
        return VOICE_AGENT_DEFAULT_PORT
    ...
    try:
        ...
        pass  # exists but not ours — still a live process
    except Exception:
        return VOICE_AGENT_DEFAULT_PORT
...
def check_voice_stack(
    ...
    voice_port = resolve_voice_agent_port()
    voice_check = check_port(voice_port, "voice-agent", probe=True)
...
def _voice_log_path() -> Path:
    ...
    THREE paths exist for legitimate reasons — one per launcher:
    - launchd plist (~/Library/LaunchAgents/com.sutando.voice-agent.plist)
      ...
      installer** — not a fixed assumption.
    - `src/startup.sh` writes to `<workspace>/logs/voice-agent.log`
      (`LOGS_DIR="$WORKSPACE/logs"`) when started via startup.sh.
    - `src/deploy.sh` runs with cwd=repo and passes the *relative*
      `logs/voice-agent.log` to `start_detached`, so a deploy.sh-launched
      agent logs to `<repo>/logs/voice-agent.log`. REPO_DIR is where `logs/`
      lives per this module's own WORKSPACE_DIR comment — the resolver just
      never learned about it, so `voice-watchers` and `voice-transport` both
      warned "voice-agent.log not found" on every deploy.sh host while a
      perfectly good 9KB log sat in the repo (observed 2026-08-06 on
      xyfu-mbp: agent healthy on :7980, banner + all 3 watchers + `setup
      complete` all present in <repo>/logs/voice-agent.log, both probes
      blind). That is the failure mode voice-transport exists to catch, so
      the miss silently disabled the only probe that can see a server-side
      1007/1011/1006 reject.

    **Selection is newest-mtime-among-candidates-with-content**, not a fixed
    priority order. Same reasoning as the voice-agent *port* resolver: the
    running process is the authority, so a stale log from a previous
    launcher must never outrank the live one. A host that once ran under
    Sutando.app and now runs deploy.sh keeps an old app-support log forever;
    strict priority would pin the probes to that corpse and report
    stale-but-green. mtime is the same liveness signal every other reader in
    this repo trusts (`state/cores/<host>.alive`, cron-runner state).

    Falls back to the workspace path when no candidate has content, so the
    "not found" detail still names a stable, expected location.
    """
    candidates = [
        Path.home() / "Library/Application Support/Sutando/logs/voice-agent.log",
        WORKSPACE_DIR / "logs" / "voice-agent.log",
        REPO_DIR / "logs" / "voice-agent.log",
    ]
    live = []
    for c in candidates:
        try:
            ...
            # Missing, or unreadable (perms/dangling symlink) — not a
            # candidate. Never raise: health-check must degrade to the
            # default, never abort the whole run.
            continue
        if st.st_size > 0:
            live.append((st.st_mtime, c))
    if live:

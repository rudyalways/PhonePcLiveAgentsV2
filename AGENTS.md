...
**Screen capture** — see what's on the user's screen. The screen-capture server runs on port 7900 (started by `src/deploy.sh` or `python3 src/screen-capture-server.py`):
```bash
